...
This install the `asysimg` script.
"""
...
"""
    uninstall(;verbose::Bool = true, dry::Bool = false)
...
function uninstall(dir = _default_install_dir();verbose::Bool = true, dry::Bool = false)
    ...
    if verbose && isfile(script_file)
        @info "AutoSysimages: Removing `$script_file`"
    end
    dry || rm(script_file; force = true)
    cleanup(;verbose = verbose, dry = dry)
end

"""
    cleanup(;verbose::Bool = true, dry::Bool = false)

This removes files generated by this packages, including the generated sysimages and collected precompile statements. 
 
The only exception is preference files `SysimagePreferences.toml` stored next to corresponding `Project.toml`.
"""
function cleanup(;verbose::Bool = true, dry::Bool = false)
    function rm_fce(file)
        if isfile(file)
            verbose && @info " - `$file`"
            dry || rm(file)
        end
    ...
    ispath(asysimg_dir) || return
    for version_dir = readdir(asysimg_dir; join = true)
    ...
    dry || rm(asysimg_dir, recursive = true)
end
...
function _append_statements(precompiles::Vector{String})
    active_dir() # Make sure the directory exists
